...
from gestion_usb.models import db, Usuario, Rol, Cargo, Equipo
...
def lista_usuarios():
    usuarios = Usuario.query \
        ...
        .add_entity(Rol).add_entity(Cargo).add_entity(Equipo).all()

    usuarios_con_relaciones = []
    for usuario, rol, cargo, equipo in usuarios:
        usuario.rol = rol
        usuario.cargo = cargo
...
def crear_usuario():
    ...
    return render_template('crear_usuario.html', roles=roles, cargos=cargos, equipos=equipos)
...
def editar_usuario(usuario_id):
    ...
    return render_template('editar_usuario.html', usuario=usuario, roles=roles, cargos=cargos, equipos=equipos)

@usuarios_bp.route('/usuarios/eliminar/<int:usuario_id>', methods=['POST']) #metodo eliminar usuario
